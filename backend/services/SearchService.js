...
class SearchService {
    ...
    async getSearchInCourses(searchQuery) {
        let query = await this.knex
            .select("courses.id as id", "courses.name as name", "courses.picture as picture", "courses.created_at as created_at", "institutions.name as institutionName")
            .from("courses")
    ...
    async getSearchInClasses(searchQuery) {
        let query = await this.knex
            .select("classes.id as id", "classes.name as name", "classes.created_at as created_at", "courses.name as courseName")
            .from("classes")
    ...
    async getSearchInNotes(searchQuery) {
        let query = await this.knex
            .select("notes.id as id", "notes.name as name", "notes.type as type", "notes.created_at as created_at", "users.name as userName", "classes.name as className")
            .from("notes")
